...
function gameLoop() {
  ...
  playerTablePlaceholder.dataset.name = "CPU esta atacando...";
  cpuTablePlaceholder.dataset.name = "Jugador esta atacando...";
...
function sinkingEvent(x, y, gb) {
  ...
  gameStatus.textContent = `${
    isPlayerTurn ? "Jugador" : "CPU"
  } se hundió ${isPlayerTurn ? "CPU" : "Jugador"} ${
    gb.getBoardArray()[x][y].type
  }.`;
...
function gameOverEvent(winner) {
  ...
  const gameStatus = document.querySelector(".game-status");
  gameStatus.textContent = winner === "player" ? "Ganaste!" : "CPU ha ganado.";
...
function placeBlocks() {
  ...
  placementInfo.textContent =
    "Arrastrar los bloques al tablero. Clickea para rotar los bloques.";
  placementInfoPara.textContent =
    "Clickea 'Iniciar Juego' sin colocar bloques para colocarlos aleatoriamente";
